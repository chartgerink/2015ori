...
We analyzed the genuine- and fabricated data sets (36 and 39, respectively) for each of the anchoring studies using four types of analyses. 
...
Each analysis was run using 100,000 iterations, from which we computed a bootstrapped $p$-value.
...
Assuming homogeneous populations of variances is unrealistic for the very different anchoring conditions or studies. 
...
Figure \@ref(fig:ddfab_density1) shows a group-level comparison of the genuine- ($k=36$) and fabricated ($k=`r count1`$) datasets, which contains four $p$-values and relevant effect sizes ($r$) for each type of effect (gender, anchoring, interaction) per dataset (i.e., $75\times4$ data points for each plot). 
